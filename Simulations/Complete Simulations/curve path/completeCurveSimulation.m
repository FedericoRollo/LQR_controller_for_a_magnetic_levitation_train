...
ydot  = vy;
vydot=(-G4+G1-G5+G2-G6+G3+m*g*sin(phiy+alpha)-m*radius*(wpsi^2)*cos(phiy+alpha))/m - vx*psidot;% + vz*phidot -> si toglie
...
vql_ref    = 0;
ig_ref = 100;
G4_ref = (mu0*Ng^2*Ag/4)*(ig_ref/0.02)^2;
G5_ref = G4_ref;
G6_ref = G4_ref;
G1_ref = (1/3)*(-m*g*sin(alpha)+m*radius*(wpsi^2)*cos(phiy+alpha))+(G4_ref);
ig1_ref = sqrt(G4_ref*(4/(mu0*Ng^2*Ag)))*ymax;
...
% diff equations
z_dot    = zdot;
zdot_dot = (L1 + L2 + L3 + L4 + L5 + L6 - (rho*Ss*Cl/2)*(xdot_ref)^2 - m*g*cos(phi+alpha)*cos(theta) - m*radius*(thetadot^2)*sin(phi+alpha))/m;% +xdot_ref*thetadot
...
iri_ref = sqrt( (1/(6*(mu0*Nr^2*Al/4)))*((rho*Ss*Cl/2)*(xdot_ref)^2 + m*g*cos(phi_ref+alpha) + m*radius*(thetadot_ref^2)*sin(phi_ref+alpha))*zi_ref^2 -m*xdot_ref*thetadot_ref);
L2_ref = (mu0*Nr^2*Al/4)*(iri_ref/zi_ref)^2;
L5_ref = L2_ref;
L1_ref = -(1/2)*L2_ref -(1/4)*((zA/xB)*(rho*Ss*Cd/2)*xdot_ref^2 - (xA/xB)*(rho*Ss*Cl/2)*xdot_ref^2 - (zB/xB)*(P1+P2+P3+P4+P5+P6)) + (1/4)*(rho*Ss*Cl/2)*xdot_ref^2 + (1/4)*m*g*cos(phi_ref+alpha)*cos(theta_ref); %-(1/4)*xdot_ref*thetadot_ref;
L4_ref = L1_ref;
